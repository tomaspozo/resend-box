...
program
  .name('resend-box')
  .description(
    'Local Resend API mock and SMTP server with web UI for email testing'
  )
  .version('0.1.0')
  .addHelpText(
    'after',
    ...
    try {
      ...
      const server = app.listen(httpPort, () => {
        ...
        if (uiExists) {
          ...
          console.log(
            `🌐 Web UI:     (not built - run 'npm run build' to build it)`
          )
        }
      ...
      server.on('error', (error: NodeJS.ErrnoException) => {
        if (error.code === 'EADDRINUSE') {
          console.error(
            `❌ Port ${httpPort} is already in use. Try a different port with --http-port`
          )
        } else {
